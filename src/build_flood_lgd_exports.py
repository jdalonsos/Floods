...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create copies of the checked JRC and Gaspar workbooks and append a "
            "FLOOD_LGD sheet based on the current LGD mapping."
        )
    )
    ...
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR), help="Folder where the copied workbooks will be written.")
    parser.add_argument("--sheet-name", default=DEFAULT_SHEET_NAME, help="Name of the derived sheet to append.")
    parser.add_argument("--replace-sheet", action="store_true", help="Replace the target sheet if it already exists in the copied workbook.")
...
def write_sheet(workbook_path: Path, output_path: Path, sheet_name: str, rows: list[list[Any]], replace_sheet: bool) -> None:
    ...
    for column_letter in ("C", "H", "I"):
        for cell in worksheet[f"{column_letter}2:{column_letter}{max_row}"]:
            for item in cell:
...
def build_output_name(input_path: Path, sheet_name: str) -> str:
    safe_sheet_name = sheet_name.replace(" ", "_")
    return f"{input_path.stem}_with_{safe_sheet_name}{input_path.suffix}"
...
def process_target(
    ...
    *,
    ...
    write_sheet(target.input_path, output_path, sheet_name, rows, replace_sheet=replace_sheet)
    print(f"Wrote {output_path} with {len(rows)} {sheet_name} rows from {target.source_label}.")
    return output_path
...
def main() -> None:
    ...
    targets = [
        FloodTarget(
            input_path=jrc_workbook,
            output_name=build_output_name(jrc_workbook, args.sheet_name),
            source_label="JRC",
        ),
        FloodTarget(
            input_path=gaspar_workbook,
            output_name=build_output_name(gaspar_workbook, args.sheet_name),
            source_label="GASPAR",
        ),
    ]
